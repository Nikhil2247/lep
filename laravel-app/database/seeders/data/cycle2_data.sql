-- Cycle 2 catalog import (cycles/projects/project_tasks), run by
-- Cycle2DataSeeder. Extracted from the reviewed, hash-verified
-- lep_cycle2_import_FINAL_PRODUCTION_v1.2.sql, executable body only
-- (STEP 1-4: preflight, baseline, INSERTs, verify-then-commit-or-rollback
-- gate). The original file's doc header and post-commit cosmetic report
-- (which contains a semicolon inside a string literal, unsafe for naive
-- statement splitting) are intentionally omitted - Cycle2DataSeeder
-- reproduces that reporting itself via DB::select() on @import_ok /
-- @finish_sql after running this script. See the original .sql for the
-- full runbook and safety rationale. Idempotent: safe to re-run.
SET NAMES utf8mb4;
