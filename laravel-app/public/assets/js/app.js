 ...
 *
 * Ported unchanged from the legacy assets/js/app.js except for the
 * request URLs, which now point at Laravel routes instead of the old
 * ajax/*.php / download_project.php scripts.
 */
(function () {
    'use strict';
    ...
    if (districtSel) {
        districtSel.addEventListener('change', async function () {
            ...
            try {
                const res = await fetch(`/ajax/blocks?district_id=${did}`);
    ...
    if (blockSel) {
        blockSel.addEventListener('change', async function () {
            ...
            try {
                const res = await fetch(`/ajax/schools?block_id=${bid}`);
                const json = await res.json();
                if (!json.success) throw new Error(json.message || 'Failed');
    ...
    if (gradeSel) {
        gradeSel.addEventListener('change', async function () {
            ...
            try {
                const res = await fetch(`/ajax/subjects?grade_id=${gid}`);
                const json = await res.json();
                if (!json.success) throw new Error(json.message || 'Failed');
    ...
    async function loadProject() {
        ...
        try {
            const res = await fetch(`/ajax/project?${params.toString()}`);
            const json = await res.json();
