{{-- Ported from the legacy index.php (public form + certificate) --}}
@extends('layouts.app')
...
@if ($success)
    ...
    @php
        $fmtDate = $certificate && $certificate->submitted_at
            ? date('d F Y', strtotime($certificate->submitted_at))
            : '';
    @endphp
        ...
        <div class="certificate-card">
            <div class="certificate-inner">
                ...
                <div class="cert-name-line">
                    ...
                    <span class="cert-name-value">{{ $certificate->teacher_name }}</span>
                </div>

                <div class="cert-compact-rows">
                    <div class="cert-row-inline">
                        <span class="cert-pair"><span class="cert-inline-label">SCHOOL:</span> <span class="cert-inline-value">{{ $certificate->school->school_name }}</span></span>
                        <span class="cert-pair"><span class="cert-inline-label">UDISE CODE:</span> <span class="cert-inline-value">{{ $certificate->school->udise_code }}</span></span>
                    </div>
                    <div class="cert-row-inline">
                        <span class="cert-pair"><span class="cert-inline-label">GRADE:</span> <span class="cert-inline-value">{{ $certificate->grade->name }}</span></span>
                        <span class="cert-pair"><span class="cert-inline-label">SUBJECT:</span> <span class="cert-inline-value">{{ $certificate->subject->name ?? 'School Leaders' }}</span></span>
                    </div>
                    <div class="cert-row-inline cert-row-full">
                        <span class="cert-pair cert-pair-full"><span class="cert-inline-label">PROJECT TITLE:</span> <span class="cert-inline-value">{{ $certificate->project->project_title }}</span></span>
                    </div>
                    <div class="cert-row-inline cert-row-sid">
                        <span class="cert-pair"><span class="cert-inline-label">SUBMISSION ID:</span> <span class="cert-inline-value cert-sid">{{ $certificate->submission_id }}</span></span>
                        <span class="cert-pair"><span class="cert-inline-label">DATE:</span> <span class="cert-inline-value">{{ $fmtDate }}</span></span>
                    </div>
                </div>
                ...
                <div class="text-center py-2">
                    <div class="cert-sid">{{ $success }}</div>
...
@else
    ...
    <!-- ===================== MAIN FORM ===================== -->
    <form id="lepForm" method="POST" action="{{ route('submissions.store') }}" enctype="multipart/form-data" novalidate>
        @csrf
                ...
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" class="form-control" id="teacher_name" name="teacher_name"
                               value="{{ old('teacher_name') }}" required maxlength="150">
                    </div>
                    <div class="col-12 col-md-6">
                        <label for="designation" class="form-label required">Designation</label>
                        <input type="text" class="form-control" id="designation" name="designation"
                               value="{{ old('designation') }}" required maxlength="100"
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <select class="form-select" id="cycle_id" name="cycle_id" required>
                            ...
                            @foreach ($cycles as $c)
                                <option value="{{ $c->id }}" @selected(old('cycle_id') == $c->id)>
                                    {{ $c->name }}
                                </option>
                            @endforeach
                        </select>
