 ...
 * Reads the first worksheet using ZipArchive + SimpleXML.
 * Suitable for simple tabular master-data files.
 */
...
class SimpleXlsxReader
{
    ...
    private function parse(): void
    {
        ...
        // First worksheet path from workbook
    ...
    private function loadSheet(string $xml): void
    {
        ...
        foreach ($rows as $row) {
            ...
            // Build contiguous row array from 0..maxCol
            $rowArr = [];
            for ($i = 0; $i <= $maxCol; $i++) {
                $rowArr[$i] = $cells[$i] ?? '';
            }
